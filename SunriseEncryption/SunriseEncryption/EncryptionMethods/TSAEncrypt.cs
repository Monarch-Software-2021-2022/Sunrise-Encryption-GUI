...
namespace NewEncryptionMethod
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            Keys key = new Keys(encryptionKey);
            List<char> KeyList = key.GetCorrespondingList();

            string unencryptedMessage = "Hello World";
            string emptyMessage = "";
            foreach (char letter in unencryptedMessage.ToLower())
            {
            ...
            Console.WriteLine($"Raw Message: {unencryptedMessage}\nEncrypted Message: {emptyMessage}\n");
            Console.WriteLine("Caesar Key Shift : " + key.CalculateShift(emptyMessage) + " Our Key : " + encryptionKey);
            Console.WriteLine($"Caesar Encryption : {CasesarShift.Encrypt(emptyMessage, key.CalculateShift(emptyMessage))}");

            Console.WriteLine("\n\nPress ENTER to Decrypt Message");
            Console.ReadLine();
            ...
            int encryptedKey = int.Parse(Console.ReadLine());

            int reversedShift = Keys.ReverseShift(encryptedMessage, encryptedKey);
            string decryptedCaesar = CasesarShift.Decrypt(encryptedMessage, reversedShift);

            Keys ah = new Keys(encryptedKey);
            List<char> getList = ah.GetCorrespondingList();

            string newMessage = "";
            foreach (char letter in decryptedCaesar.ToLower())
            {
                if (char.IsLetter(letter))
                {
                    int index = getList.IndexOf(letter);
    ...
    {
        public static List<char> Shuffle(this List<char> oldList)
        {
            Random rand = new Random();
            return oldList.OrderBy(item => rand.Next()).ToList();
        }
    }
